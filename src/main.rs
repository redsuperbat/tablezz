...
fn commands_markdown() -> String {
    ...
    let scopes: Vec<(&str, Vec<(String, Option<String>, String, Option<String>)>)> = vec![
        (
            "always",
            builtin::global()
                .into_iter()
                .map(|(command, keybind)| {
                    (
                        ...
                        command.args_hint(),
                        keybind.map(str::to_string),
                    )
                })
                .collect(),
        ),
        ("visual mode", scoped(builtin::visual_enter())),
        ("visual mode", scoped(builtin::visual_exit())),
        ("picker", scoped(builtin::picker())),
        ("command line", scoped(builtin::command_line())),
        ("autocomplete", scoped(builtin::autocomplete())),
        ("help", scoped(builtin::help())),
        ("help search", scoped(builtin::help_search())),
    ];
    ...
    for (scope, commands) in scopes {
        for (name, description, args, keybind) in commands {
            ...
            rows.push(format!(
                "| {name} | {description} | {args} | {keybind} | {scope} |"
            ));
        }
    }
...
fn scoped(
    entries: Vec<(commands::Command, keybinds::Keybind)>,
    ...
    entries
        ...
        .map(|(command, keybind)| {
            (
                ...
                command.description.clone(),
                command.args_hint(),
                Some(keybind.keybind_expression),
            )
        })
        .collect()
}
